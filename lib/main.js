...
proto.match = function(method, url){
	return this.map[method].some(function(handle){
		switch(handle.re.constructor.name){
			case "String":
				return handle.re === url;
			case "RegExp":
				return handle.re.test(url);
		}
	});
};
